...
class FSRCNN(object):
    def __init__(self,sess,config):
        ...
        scale_factor = [[14,20],[11,21],[10,24]]
        self.image_size,self.label_size = scale_factor[self.scale - 2]
        
        
        stride_factor = [10,7,6]
        if not self.train:
            self.stride = stride_factor[self.scale - 2]
        
        self.model_params = [56, 12, 4]
        
        self.checkpoint_dir = config.checkpoint_dir
        self.output_dir = config.output_dir
        self.data_dir = config.data_dir
        self.pic_idx = config.pic_idx
    ...
    def model(self):
        ...
        #shrinking
        conv_shrinking = self.prelu(tf.nn.conv2d(conv_feature,self.weights['w2'],strides=[1,1,1,1],padding='SAME') + self.biases['b2'],2)
        #mapping
        prev_layer,m = conv_shrinking,self.model_params[2]
        for i in range(3,m + 3):
            weight,bias = self.weights['w{}'.format(i)],self.biases['b{}'.format(i)]
            prev_layer = self.prelu(tf.nn.conv2d(prev_layer,weight,strides=[1,1,1,1],padding='SAME') + bias,i)
        #expanding
        expand_weight,expand_bias = self.weights['w{}'.format(m + 3)],self.biases['b{}'.format(m + 3)]
        conv_expanding = self.prelu(tf.nn.conv2d(prev_layer,expand_weight,strides=[1,1,1,1],padding='SAME') + expand_bias, 7)
        
        #Deconvolution
        deconv_output = [self.batch,self.label_size,self.label_size,self.c_dim]
        deconv_stride = [1,self.scale,self.scale,1]
        deconv_weight,deconv_bia = self.weights['w{}'.format(m + 4)],self.biases['b{}'.format(m + 4)]
        conv_deconv = tf.nn.conv2d_transpose(conv_expanding,
                                             deconv_weight,
                                             output_shape=deconv_output,
                                             strides=deconv_stride,
                                             padding='SAME') + deconv_bia
        return conv_deconv
